...
class Product extends Model implements HasMedia
{
    ...
    {
        if ($this->quantity == 0) {
            return '<span class="text-warning">' . 'Run Out' . '</span>';
        } elseif ($this->quantity == -1) {
            return '<span class="text-danger">' . 'Out OF Sold' . '</span>';
        } else {
    ...
    public function showPrice()
    {
        if ($this->getPrice() == 0) {
            return 'Call us !';
        } elseif ($this->getPrice() == -1) {
            return '-';
        }

        return number_format($this->getPrice()) . ' ' . config('app.currency');
